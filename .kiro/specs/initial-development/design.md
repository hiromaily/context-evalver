# Technical Design: context-optimizer (initial-development)

---

## Overview

**Purpose**: `context-optimizer` is a Claude Code plugin that silently observes developer session behavior, persists events to SQLite, and proposes repository context improvements (CLAUDE.md updates, Skills, slash commands) only when statistical evidence meets confidence thresholds.
...
graph TB
    ...
    subgraph Plugin[context-optimizer Plugin]
...
- Draft patches generated by `/context-draft` are staged to `~/.local/share/context-optimizer/drafts/{session_id}.json` so `/context-apply` can read them in a subsequent ephemeral invocation. The staging file is cleaned up by the `SessionEnd` hook.
...
| Rust edition | Edition 2024 | Core binary | `context-optimizer-core` binary |
...
| IPC channel | Unix domain socket + JSONL | TS ↔ Rust communication | Socket at `~/.local/share/context-optimizer/{session_id}.sock` |
...
sequenceDiagram
    ...
    SS->>Daemon: spawn context-optimizer-core --session-id {session_id}
    Daemon->>FS: open / init SQLite DB
    Daemon->>FS: bind Unix socket at ~/.local/share/context-optimizer/{session_id}.sock
    SS-->>CC: exit 0
...
sequenceDiagram
    ...
    Dev->>Skill: /context-optimizer:context-audit
...
sequenceDiagram
    ...
    Dev->>Draft: /context-optimizer:context-draft
    ...
    Dev->>Apply: /context-optimizer:context-apply
...
| ConfigLoader | TS / Shared | Load and validate `.context-optimizer.json` | 12.1–12.5 | — |
...
- Checks `.context-optimizer-ignore` existence at `cwd` root (1.8)
...
**Responsibilities & Constraints**
- Derives socket path deterministically from `session_id` as `~/.local/share/context-optimizer/{session_id}.sock`; `session_id` is read from the hook stdin payload or passed explicitly by command scripts
...
// Path: ~/.local/share/context-optimizer/drafts/{session_id}.json
...
| Field | Detail |
|-------|--------|
| Intent | Loads, validates, and provides defaults for `.context-optimizer.json` |
...
**Responsibilities & Constraints**
- Opens/creates DB at `~/.local/share/context-optimizer/db/{repo_root_hash}.db`
...
- DB file path: `~/.local/share/context-optimizer/db/{sha256(repo_root)[..16]}.db`
...
- Daemon logs prefixed with `[context-optimizer]` for filtering
...
2. Run 3 simulated sessions against a test repo; invoke `/context-optimizer:context-audit`; verify report generated
...
- **Opt-out**: `.context-optimizer-ignore` check is the first action in `HookDispatcher`; if present, process exits 0 immediately.
